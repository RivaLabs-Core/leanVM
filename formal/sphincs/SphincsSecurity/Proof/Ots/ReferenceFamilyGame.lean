...
def fixedReferenceDummyWord : Encoding := Checksum.encode 0

theorem fixedReferenceDummyWord_valid : Checksum.Valid fixedReferenceDummyWord :=
  Checksum.encode_valid 0
...
theorem referenceFamilyOracleSample_words_valid (key : SecretKey) (inputs : Finset HashInput)
    ...
    ∀ lay tree leaf, Checksum.Valid (referenceFamilyWords result.1 dummy lay tree leaf) := by
