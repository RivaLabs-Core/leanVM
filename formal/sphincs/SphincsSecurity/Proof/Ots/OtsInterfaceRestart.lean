...
import SphincsSecurity.Proof.Ots.OtsTraceCheckpointInterfaceBudget
import SphincsSecurity.Proof.Ots.OtsContactEvents

namespace SphincsSecurity.Concrete.OtsPrefix

open OracleComp OracleSpec PartialChainEndpoint
...
theorem instrumentedCheckpoint_newContact_le_of_counted (budget : Nat)
  ...
  have hkernel := realCheckpointRun_contact_charge
    ...
    (fun endpoint middle hmiddle _ result hresult =>
      (segment.traceCheckpoint_budget_of_counted stop inputs hencoding hgraph auxiliary secrets ftsSecret words adversary budget hreal hsmall
...
theorem instrumentedCheckpoint_newContact_le_mark_of_counted (budget : Nat)
    (hreal : ∀ result ∈ (realRun (fun _ => uniformImpl)
      (fun endpoint => QueryCap.counted IsPrefixQuery
        (segment.seedGame inputs hencoding hgraph auxiliary secrets ftsSecret words endpoint adversary)) (fun _ _ => none)).support,
      result.2.1.2 ≤ budget)
  ...
  have hkernel := realCheckpointRun_contact_le_mark aux before after (fun _ _ => none) marked budget (fun _ _ h => h.2)
    ...
    (fun result hresult _ => by
      ...
      exact (segment.traceCheckpoint_budget_of_counted stop inputs hencoding hgraph auxiliary secrets ftsSecret words adversary budget hreal hsmall
...
theorem contactSeed_newContact_le_of_counted (budget : Nat)
    (hreal : ∀ result ∈ (realRun (fun _ => OtsPrefix.uniformImpl)
      (fun endpoint => QueryCap.counted IsPrefixQuery
        ((OtsPrefix.atAddress parameter words address).seedGame inputs hencoding hgraph auxiliary secrets ftsSecret words endpoint adversary))
      (fun _ _ => none)).support, result.2.1.2 ≤ budget)
    (hsmall : budget < Fintype.card Digest) :
  ...
  simpa only [ContactResult.Marked, checkpointObserver_contact] using
    segment.instrumentedCheckpoint_newContact_le_of_counted OtsContactTrace.Stopped inputs hencoding hgraph auxiliary secrets ftsSecret words adversary
      budget hreal hsmall

theorem checkpointSeed_newContact_le_mark_of_counted (stop : FrontierStop)
  ...
  rw [hevent]
  exact segment.instrumentedCheckpoint_newContact_le_mark_of_counted stop inputs hencoding hgraph auxiliary secrets ftsSecret words adversary budget hreal hsmall
