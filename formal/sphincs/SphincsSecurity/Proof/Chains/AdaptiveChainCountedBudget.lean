...
include hbound hsmall

theorem lazyRun_counted_le_of_counted_real (endpoint : State) (result : (Result × Nat) × (Fin n → State → Option State))
  ...
  obtain ⟨finished, hfinished⟩ := idealRun_cap_finished_of_counted auxiliary computation budget hbound hsmall (endpoint, capped) hsource
  change capped.1 = some finished at hfinished
  rw [hfinished]
  exact Option.some_ne_none finished

theorem lazyRun_pause_budget_of_counted_real (endpoint : State) (stop : Memory → Prop) [DecidablePred stop]
  ...
  have htotal := lazyRun_counted_le_of_counted_real auxiliary computation budget hbound hsmall endpoint _ hfull
