...
/-!
The forced FTS near-certificate game is bounded slot by slot. The retained residual certificate monitor follows a forced run using the keygen cost, the fixed-length proposal word and the prefix stop rule. A near certificate on an unstopped monitor is a banked certificate, so its probability is at most the expected creation cost, which the proposal-word martingale bounds by the budget times the average terminal certificate price. A stopped monitor is a cache exception or a prefix exception, each of which is rare. The bound `nearCertificateBound` sums the two over the fourteen omitted trees.
-/
...
theorem near_alive_le (budget : Nat) (adversary : Adversary) (omitted : FtsTree) (spent : Nat) (stopped : Bool) (total : Nat)
    (hbudget : budget ≤ (2 ^ 127 + 2 ^ 64)) (hpool : stopped = false → fixedProposalLength ≤ total)
...
include hauxiliary in
theorem near_stopped_le (budget : Nat) (adversary : Adversary) (required : Finset FtsTree) (total : Nat) (hbudget : budget ≤ (2 ^ 127 + 2 ^ 64))
...
include hauxiliary in
theorem near_omitting_total_le (budget : Nat) (adversary : Adversary) (omitted : FtsTree) (total : Nat) (hbudget : budget ≤ (2 ^ 127 + 2 ^ 64))
    (hcovered : CoveredRun parameter root otsSecret inputs (adversary.main ⟨root, parameter⟩) (nearStart keygenHashCost (decide (total < fixedProposalLength))))
    (hwork : ∀ result : Completed × CachedState,
      nearLaw parameter root otsSecret labels inputs hencoding selections rows dummy slot adversary result ≠ 0 →
        keygenHashCost + completedWork result.1 ≤ budget) :
...
include hauxiliary in
theorem near_omitting_le (budget : Nat) (adversary : Adversary) (omitted : FtsTree) (hbudget : budget ≤ (2 ^ 127 + 2 ^ 64))
...
include hauxiliary in
theorem nearLaw_certificate_le (budget : Nat) (adversary : Adversary) (hbudget : budget ≤ (2 ^ 127 + 2 ^ 64))
    (hcovered : ∀ monitor, CoveredRun parameter root otsSecret inputs (adversary.main ⟨root, parameter⟩)
      ((∅, initialState PUnit.unit), monitor))
    (hwork : ∀ result : Completed × CachedState,
      nearLaw parameter root otsSecret labels inputs hencoding selections rows dummy slot adversary result ≠ 0 →
