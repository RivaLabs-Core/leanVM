...
theorem Context.words_valid {inputs : Finset HashInput} (context : Context inputs)
    ...
    ∀ lay tree leaf, Checksum.Valid (context.words lay tree leaf) := by
...
theorem Compatible.layer_word {inputs : Finset HashInput} {context : Context inputs} {memory : Memory}
    ...
    (hword : Checksum.Valid (context.words lay tree leafIdx))
  ...
  apply Eq.symm
  apply Checksum.eq_of_le_of_valid hword (valid_of_eval_encode_eq_some _ _ _ _ _ _ _ _ hencode)
  intro chain
  by_contra hnot
