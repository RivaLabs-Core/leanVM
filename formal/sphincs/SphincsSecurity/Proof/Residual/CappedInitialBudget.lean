import SphincsSecurity.Proof.Residual.HonestWorkBound
...
theorem observedInitialCappedSource_interfaceHashes_le (parameter : PublicParameter) (_hparameter : parameter ∈ support sampleParameter)
    ...
    (hinputs : ∀ key : SecretKey, sourceInputs key (signingCap (unloggedRetainedRestComputation (Seeded.memoAdversary (Security.embed original)) ⟨key.root, key.parameter⟩)) ⊆ inputs)
    (q : Nat) (hsmall : q < 2 ^ 256) (hq : Security.HasHashQueryBound original q) (result : Option (Option ((Forgery × Bool) × Nat)) × State inputs)
    (hresult : observedRun
      ...
      (simulateQ (adversaryImpl inputs parameter (knownRoot (initialKnown (referenceFamilyWords encoding.selections dummy) exposed))
        ...
        (signingCap (unloggedRetainedRestComputation (Seeded.memoAdversary (Security.embed original)) ⟨knownRoot (initialKnown (referenceFamilyWords encoding.selections dummy) exposed), parameter⟩)))
  ...
  have hsource := context.capped_interface_bound hroot original q hsmall hq
  ...
  have hrun : observedRun context.environment context.actual context.auxiliary.seed
      (simulateQ (adversaryImpl inputs context.key.parameter context.key.root context.words context.auxiliary.selections)
        (signingCap (unloggedRetainedRestComputation (Seeded.memoAdversary (Security.embed original)) ⟨context.key.root, context.key.parameter⟩)))
  ...
  have h := observedRun_source_interfaceHashes_le context (signingCap (unloggedRetainedRestComputation (Seeded.memoAdversary (Security.embed original)) ⟨context.key.root, context.key.parameter⟩))
...
theorem lazyInitialCappedSource_interfaceHashes_le (parameter : PublicParameter) (hparameter : parameter ∈ support sampleParameter)
    ...
    (result : Option (Option ((Forgery × Bool) × Nat)) × State inputs)
    (hresult : lazyRun
  ...
  exact observedInitialCappedSource_interfaceHashes_le parameter hparameter inputs
