...
theorem checkedHashQuery_interface_payment (routing : Routing) (input : inputs) (state : State inputs) (q : Nat)
    ...
    (hresources : InterfaceResources state.memory) (hquery : state.memory.interfaceHashes + 1 ≤ q)
    ...
    probability + (1 - probability) *
      ((if FtsProbeSimulation.MessageHashInput parameter input.val then (2 ^ digestBits : ℝ)⁻¹ else 0) +
        PrimitiveMessagePotential.value (2 ^ digestBits) afterProbes ((q : ℝ) - (state.memory.interfaceHashes + 1))) ≤
      PrimitiveMessagePotential.value (2 ^ digestBits) state.memory.external.probes ((q : ℝ) - state.memory.interfaceHashes) := by
  have hspace : (0 : ℝ) < 2 ^ digestBits := by positivity
  have hp : state.memory.external.probes ≤ state.memory.interfaceHashes := by
    exact (Nat.le_add_right _ _).trans hresources.2.2
  have hpReal : (state.memory.external.probes : ℝ) ≤ state.memory.interfaceHashes := by exact_mod_cast hp
  have hqReal : (state.memory.interfaceHashes : ℝ) + 1 ≤ q := by exact_mod_cast hquery
  have hbudgetReal : 2 * (q : ℝ) ≤ 2 ^ digestBits := by exact_mod_cast hbudget
  have hremaining : (0 : ℝ) ≤ q - (state.memory.interfaceHashes + 1) := by linarith
  have hsum : (state.memory.external.probes : ℝ) + (q - (state.memory.interfaceHashes + 1)) + 1 < 2 ^ digestBits := by
    linarith
  have hmin : state.memory.external.probes < 2 ^ digestBits := by
    have hpos : 0 < 2 ^ digestBits := by positivity
    omega
  have hleft : ((q : ℝ) - (state.memory.interfaceHashes + 1)) + 1 = q - state.memory.interfaceHashes := by ring
  dsimp only
  ...
  · rw [prob_checkedHashQuery_message_stop parameter inputs hencoding words publicReplies selections rows routing input state hmessage hcovered,
    ...
    have h := PrimitiveMessagePotential.message_payment (2 ^ digestBits) state.memory.external.probes
      ((q : ℝ) - (state.memory.interfaceHashes + 1)) hspace (by positivity) hremaining (by linarith)
  ...
  · rw [if_neg hmessage, zero_add]
    ...
    have h := PrimitiveMessagePotential.nonmessage_step (2 ^ digestBits) state.memory.external.probes _
      ((q : ℝ) - (state.memory.interfaceHashes + 1)) _ hremaining hpaid hsum hprobReal
