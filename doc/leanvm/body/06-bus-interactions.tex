...
The machine runs six interactions on the bus, differentiated by their domain separators (\S\ref{sec:m3}): the VM state, the read-write memory, the public bytecode, two range arrays, and the exponentiation array. All but the first use offline memory checking~\cite{BEGKN94}: the memory in its read-write form, ordered by a clock (\S\ref{sec:memchan}), and the four read-only arrays in the simpler form of \S\ref{sec:lookup}.

Two encodings of an integer $i$ coexist on the bus. What the machine indexes (a memory cell, an instruction) and what it holds (a register, a pointer) is the integer itself, $\intk{i}$ (\S\ref{sec:vm}). What the proof system only ever steps (a clock, a read count) is the power $\gen^{i}$, whose successor is a free multiplication by $\gen$. The exponentiation array (\S\ref{sec:exp}) translates the first into the second, which is how integers get added.

\subsection{VM State}\label{sec:state}

The state interaction uses domain separator $\dsep{ST} = \gen^{0}$, and carries the 2 registers, program counter and frame pointer, the frame pointer's $\gen$-power $\fpx$ (\S\ref{sec:exp}), and a \emph{clock} $\ts\in\K$: $\tup{\dsep{ST},\pc,\fp,\fpx,\ts}$. Each instruction row pulls its current state and pushes its successor $\tup{\dsep{ST},\nxt{\pc},\nxt{\fp},\nxt{\fpx},\gen^{s}\cdot\ts}$, where the \emph{stride} $s$ is $4$ for every instruction but \texttt{BLAKE2S}, whose stride is $20$. An honest clock is therefore $\gen^{4c}$ at cycle $c$, \texttt{BLAKE2S} counting for five cycles, which leaves each of a row's memory accesses a timestamp of its own, $\gen^{4c+k}$ for the access in \emph{slot} $k<s$ (\S\ref{sec:memchan}).

The bus is preseeded by pushing the initial state, which is cycle $1$ in frame $0$, and pulling the final state (boundary conditions):
\begin{align*}
  \tup{\dsep{ST},\pc_{\mathrm{initial}},\fp_{\mathrm{initial}},\fpx_{\mathrm{initial}},\ts_{\mathrm{initial}}} &= \tup{\dsep{ST},\intk{0},\intk{0},\gen^{0},\gen^{4}},\\
  \tup{\dsep{ST},\pc_{\mathrm{final}},\fp_{\mathrm{final}},\fpx_{\mathrm{final}},\tsfinal} &= \tup{\dsep{ST},\intk{\nprog - 1},\intk{0},\gen^{0},\tsfinal}.
...
\item \textbf{Finalize:} for each $i$, \pull\ $\tup{\dsep{MEM},\intk{i},\tsfin[i],\memfin[i]}$.
\end{itemize}
...
Let the flush rules above be the only flushes carrying $\dsep{MEM}$, let the clocks be as in Corollary~\ref{cor:clock} with every timestamp's exponent below $2^{63}$, and let every access satisfy \eqref{eq:gap} with $\glo,\ghi$ addresses of $\rlo,\rhi$. If the bus balances, then every access of a row with nonzero clock is at an address $\intk{i}$, $i<2^{\kmem}$, and, the accesses of such rows at $\intk{i}$ being taken in the order of their timestamps: the first one's $v_{\mathrm{old}}$ is $\meminit[i]$, every other's $v_{\mathrm{old}}$ is its predecessor's $v_{\mathrm{new}}$, and $\memfin[i]$ is the last one's $v_{\mathrm{new}}$, or $\meminit[i]$ if there is none.
\end{theorem}
...
Fix an address $a$ and keep the tuples at $a$, which still balance. If some real access is at $a$, the one of smallest $y$ pulls a timestamp $x<y$, which no real access at $a$ pushed: the matching push is a seed, so $a=\intk{i}$ with $i<2^{\kmem}$. At such an address, let the real accesses have timestamps $y_1<\dots<y_n$. With $y_0:=0$, the pushes carry the $n+1$ distinct timestamps $\gen^{y_0},\gen^{y_1},\dots,\gen^{y_n}$, so the pulls are $n+1$ as well, which puts the finalization in this class, and carry the same ones: $\gen^{x_1},\dots,\gen^{x_n}$ and $\tsfin[i]$ are those $n+1$ values in some order. By induction on $j$, the only $y_l$ below $y_j$ not already taken is $y_{j-1}$; so $x_j=y_{j-1}$, which leaves $\tsfin[i]=\gen^{y_n}$. A pull and the push it matches agree on the value, which is the claim.
\end{proof}
...
The bytecode is a lookup array (\S\ref{sec:lookup}) of $\nprog = 2^{\kbc}$ instructions, $\kbc \le 32$ (\S\ref{sec:e2e-bc}), instruction $i$ at the address $\intk{i}$. Bus interaction uses domain separation $\sep=\dsep{BC} = \gen^{2}$. An entry is an opcode, seven operands, and two integers the row could not derive from $\pc$ by a field operation: the successor $\intk{i+1}$, which every row but a taken \texttt{JUMP} pushes as its next $\pc$, and the return address $\intk{i+2}$ where the instruction is a \texttt{DEREF} in \texttt{deref\_pc} mode (zero elsewhere). So $\ncomp=10$: with the separator, the address and the count ahead of them, this is the widest entry on the bus at $13$ coordinates, which is what forces the four index bits, hence the $m=16$ slots of \S\ref{sec:m3}. A jump's target needs no check of its own: the row it leads to reads the bytecode there, and Corollary~\ref{cor:addrrange} makes it an instruction. Coordinate $3$ is the opcode, naming the instruction:
\begin{gather*}
...
\subsection{Integer addresses}\label{sec:exp}

An address is a sum of integers, $\fp+o$, and integer addition is no low-degree operation of $\K$. Multiplication by a power of $\gen$ is, and it adds exponents. The \emph{exponentiation array} $\expa$ turns one into the other: a lookup array (\S\ref{sec:lookup}) of $2^{\kmem}$ entries under $\dsep{EXP}=\gen^{5}$, entry $i$ sitting at the address $\intk{i}$ and holding the one component $\gen^{i}$. Both are public (\S\ref{sec:idxcol}), so only its finalize counts are committed.

\paragraph{An access.} The bytecode carries an offset $o$ as $\gen^{o}$ (\S\ref{sec:e2e-bc}), and the state carries the frame pointer twice, as $\fp$ and as $\fpx=\gen^{\fp}$ (\S\ref{sec:state}). A row accessing the cell $\loc{o}$ commits its address $a$ and reads $\tup{a,\;\fpx\cdot\gen^{o}}$ off $\expa$, the value being a product of two of its columns (\S\ref{sec:m3}); for the $k$-th cell past it, as \texttt{BLAKE2S} needs, the value is $\gen^{k}\cdot\fpx\cdot\gen^{o}$. The memory tuples of \S\ref{sec:memchan} then carry $a$.

\begin{proposition}[Addition in the exponent]\label{prop:expadd}
Let a correct read of $\expa$ name the address $a$ and the value $\gen^{f}\cdot\gen^{o}\cdot\gen^{k}$, with integers $f,o<2^{32}$ and $k<4$. Then $a=\intk{f+o+k}$ and $f+o+k<2^{\kmem}$.
\end{proposition}

\begin{proof}
The read is correct, so $a=\intk{i}$ and $\gen^{i}=\gen^{f+o+k}$ for some $i<2^{\kmem}$. Both exponents are below $2^{64}-1$, the order of $\gen$, so $i=f+o+k$.
\end{proof}

\paragraph{The frame pointer.} Along the run $r_1,\dots,r_L$ of Proposition~\ref{prop:walk}, every state has $\fp=\intk{f}$ and $\fpx=\gen^{f}$ for one $f<2^{\kmem}$. The initial state does, with $f=0$. A row other than a taken \texttt{JUMP} pushes the pair it pulled. A taken \texttt{JUMP} pushes the word $v_{\fp}$ it read from memory and a column $e_{\fp}$, having read $\tup{v_{\fp},e_{\fp}}$ off $\expa$ (\S\ref{sec:tab-jump}): by Theorem~\ref{thm:omc}, $v_{\fp}=\intk{f'}$ and $e_{\fp}=\gen^{f'}$ with $f'<2^{\kmem}$. The hypothesis of Proposition~\ref{prop:expadd} on $f$ therefore holds for every access of the run, and the one on $o$ is the bytecode's. \texttt{DEREF} treats its pointer the same way: the word $p$ it read becomes a base through one read $\tup{p,e_p}$ of $\expa$, and its target's address is read at $e_p\cdot\gen^{o_2}$ (\S\ref{sec:tab-deref}). That read is also what makes a pointer, or a loaded frame, a valid address.

\paragraph{Cost.} One committed column and one read per memory access, and one more of each for a pointer or a loaded frame. A row at clock $0$ (\S\ref{sec:e2e-pad}) need not satisfy the invariant, being on no run, but its reads of $\expa$ are correct like any other, and its memory tuples meet none of the run's (Theorem~\ref{thm:rwmem}).

\subsection{The index columns}\label{sec:idxcol}

The seed and finalization blocks run over every entry of their array, so the verifier needs the extensions of the columns $i\mapsto\intk{i}$ and $i\mapsto\gen^{i}$ at the point $\zeta$ (\S\ref{sec:leafstack}). Write $w$ for the bits of $i$. The \emph{integer index column} is linear in them, $\intk{i}=\sum_k w_k\,\intk{2^{k}}=\sum_k w_k\,x^{k}$, so its extension is that same sum at $\zeta$:
\[
  \sum_{k=0}^{\kappa-1}\zeta_k\,x^{k}.
\]
For the \emph{exponent column} $\mathrm{idx}$, $\gen^{\,i}=\prod_k(\gen^{\,2^{k}})^{w_k}$, whose $k$-th factor is $1+w_k(1+\gen^{\,2^{k}})$, which gives the efficient MLE formula:
\[
