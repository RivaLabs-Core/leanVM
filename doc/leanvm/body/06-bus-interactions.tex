...
The machine runs three interactions on the bus, differentiated by their domain separators (\S\ref{sec:m3}): the VM state, the read-only memory, and the public bytecode. The last two are lookups using offline memory checking~\cite{BEGKN94}.

\subsection{VM State}\label{sec:state}

The state interaction uses domain separator $\dsep{ST} = \gen^{0}$, and carries the 2 registers, program counter and frame pointer: $\tup{\dsep{ST},\pc,\fp}$. Each instruction row pulls its current state and pushes its successor $\tup{\dsep{ST},\nxt{\pc},\nxt{\fp}}$; the bus is preseeded by pushing the initial state $\tup{\dsep{ST},\pc_{\mathrm{initial}},\fp_{\mathrm{initial}}} = \tup{\dsep{ST},\gen^0,\gen^0}$ and pulling the final state $\tup{\dsep{ST},\pc_{\mathrm{final}},\fp_{\mathrm{final}}} = \tup{\dsep{ST},\gen^{\nprog - 1},\gen^0}$ (boundary conditions).
\begin{proposition}[Balance implies the existence of a valid execution]\label{prop:walk}
Let $\mathcal R$ be a finite multiset of rows, each pulling a state $\bpull(r)$ and pushing a state $\bpush(r)$, and suppose the state channel balances:
\[
  \mset{\bpush(r):r\in\mathcal R}\uplus\mset{(\pc_{\mathrm{initial}},\fp_{\mathrm{initial}})}
  \;=\;\mset{\bpull(r):r\in\mathcal R}\uplus\mset{(\pc_{\mathrm{final}},\fp_{\mathrm{final}})} .
\]
Then $\mathcal R$ splits into rows $r_1,\dots,r_L$ with
\[
  \bpull(r_1)=(\pc_{\mathrm{initial}},\fp_{\mathrm{initial}}),\qquad \bpush(r_i)=\bpull(r_{i+1}),\qquad \bpush(r_L)=(\pc_{\mathrm{final}},\fp_{\mathrm{final}}),
\]
and rows forming closed walks.
\end{proposition}

\begin{proof}
Build the directed multigraph $G$ on the states: one edge $\bpull(r)\to \bpush(r)$ per row, plus one edge $e$ from $(\pc_{\mathrm{final}},\fp_{\mathrm{final}})$ to $(\pc_{\mathrm{initial}},\fp_{\mathrm{initial}})$.

Fix a state $x$. Its in-degree in $G$ counts the rows with $\bpush(r)=x$, plus $e$ if $x=(\pc_{\mathrm{initial}},\fp_{\mathrm{initial}})$. Its out-degree counts the rows with $\bpull(r)=x$, plus $e$ if $x=(\pc_{\mathrm{final}},\fp_{\mathrm{final}})$. Those two counts are the multiplicities of $x$ on the two sides of the hypothesis, so they are equal: $G$ is balanced at every state.
...
\paragraph{Instance caps.} The verifier checks the announced sizes against public caps (\S\ref{sec:e2e-unrolled}): at most $2^{32}$ rows per table, at most $2^{32}$ memory cells (\S\ref{sec:memchan}) and at most $2^{32}$ bytecode instructions (\S\ref{sec:bytecode}). A row reads at most twenty-seven times (\texttt{SHA3}: twenty-six cells and its bytecode entry), so the seven tables flush $\nread\le27\cdot7\cdot2^{32}<2^{40}$ reads, far under the $2^{64}-1$ that Lemma~\ref{lem:invcnt} requires.
...
\[
  \opc{XOR}=\gen^{0},\quad \opc{MUL}=\gen^{1},\quad \opc{SET}=\gen^{2},\quad \opc{DRF}=\gen^{3},\quad \opc{JMP}=\gen^{4},\quad \opc{B2S}=\gen^{5},\quad \opc{S3}=\gen^{6}.
