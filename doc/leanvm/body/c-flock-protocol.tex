...
Flock~\cite{Flock26} proves many executions of one Boolean circuit at once. We use it for three circuits: the BLAKE2s compression, and the addition and the multiplication of two $64$-bit integers modulo $2^{64}$ (\S\ref{flock:u64}). Each circuit gets its own run of the protocol, over its own witness. Since we do not follow their protocol exactly, we describe our version here, for BLAKE2s; the other two differ in the block size $\kblock$ and in the position $\jconst$ of the constant wire alone, which \S\ref{flock:u64} gives.
...
This second condition is enforced by lincheck (\S\ref{flock:lincheck}), and position $512=\jconst$ is where the BLAKE2s circuit puts its constant wire. Since it is asked of every block, the prover pads the batch out to $2^{\kbatch}$ with copies of a genuine witness block rather than with zeros.
...
\item The verifier samples $\lcalpha$ and the parties run the $8$ quadratic lincheck rounds on \eqref{flock:eq:lincheck}. The prover sends $(s_0,\ldots,s_{63})$, and the verifier checks \eqref{flock:eq:lincheck-terminal} with one circuit walk.
\end{enumerate}
...
\subsection{The u64 circuits}\label{flock:u64}

\texttt{ADD\_U64} and \texttt{MUL\_U64} (\S\ref{sec:tab-u64}) are two more circuits in the sense of Definition~\ref{def:circuit}, with one block layout: the input $a$ at positions $[0,64)$, the input $b$ at $[64,128)$, the result at $[128,192)$, the constant wire at $\jconst=192$, then the $\wedge$ wires in the order the circuit makes them. The result bits are $\oplus$ wires, committed so that they leave the circuit. Bit $i$ of an integer sits at position $i$ of its range, so each of the three is one packed $\K$-element, equal to the memory word it stands for.

\begin{center}
\begin{tabular}{lrrrr}
\hline
Circuit & $\kblock$ & $\jconst$ & $\wedge$ wires & positions used\\
\hline
BLAKE2s & $14$ & $512$ & $14{,}720$ & \\
addition & $8$ & $192$ & $63$ & $256$\\
multiplication & $12$ & $192$ & $2{,}143$ & $2{,}336$\\
\hline
\end{tabular}
\end{center}

\paragraph{Addition.} A ripple-carry adder. With $c_0=0$, the carry into position $i+1$ is the majority of $a_i,b_i,c_i$,
\[
c_{i+1}=(a_i\oplus c_i)(b_i\oplus c_i)\oplus c_i ,
\]
one $\wedge$ wire, and the result bit $a_i\oplus b_i\oplus c_i$ is affine in the wires before it. The carry out of position $63$ falls off the modulus, which leaves $63$ products, and the block is full: $192+1+63=2^{8}$.
...
Lincheck comes first, and it is the only thing that checks anything: the zerocheck's own terminal identity defines $v_c$ instead of testing it, so every lie upstream reaches lincheck as a wrong triple $(v_a,v_b,v_c)$. Its terminal check \eqref{flock:eq:lincheck-terminal} is evaluated from the true $s$ and the matrices themselves, by the walk of \S\ref{flock:native}, so a false \eqref{flock:eq:lincheck} survives the $8$ rounds with probability at most $16/|\E|$ (Fact~\ref{fact:sumcheck}, degree two in each round). And by Lemma~\ref{lem:flock-block-diagonal} and \eqref{flock:eq:c-as-column}, \eqref{flock:eq:lincheck} says
\[
