...
\begin{itemize}
\item \textbf{Columns:} $\pc,\fp,\ts$; operands $o_A,o_B,o_C$; the destination's old value $v_C^{\mathrm{old}}$; bytecode count $r_{\mathrm{bc}}$. The three words $v_A,v_B,v_C$ are committed in $q_{\mathrm{add}}$ or $q_{\mathrm{mul}}$.
\item \textbf{Constraints:} the three accesses'. The arithmetic relation is handled by Flock.
\item \textbf{Flushes:}
  \begin{itemize}
  \item \textbf{State:} \pull\ $\tup{\dsep{ST},\pc,\fp,\ts}$, \push\ $\tup{\dsep{ST},\gen\cdot\pc,\fp,\gen^{4}\cdot\ts}$.
  \item \textbf{Bytecode:} $\rdf{\dsep{BC},\pc,r_{\mathrm{bc}},\opc{ADDU},o_A,o_B,o_C,0,0}$, with $\opc{MULU}$ for \texttt{MUL\_U64}.
  \item \textbf{Memory:} $\acc{0}\tup{\fp\cdot o_A;\,v_A}$, $\acc{1}\tup{\fp\cdot o_B;\,v_B}$, $\acc{2}\tup{\fp\cdot o_C;\,v_C^{\mathrm{old}}\to v_C}$.
  \end{itemize}
\end{itemize}
