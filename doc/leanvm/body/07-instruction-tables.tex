...
\begin{itemize}
\item \textbf{Columns:} $\pc,\fp$; operands $o_{m_0},o_{m_1},o_{m_2},o_{m_3},o_{\mathit{tail}},o_{\mathit{cap}},o_{\mathit{out}}$; one memory count per cell, twenty-six; bytecode count $r_{\mathrm{bc}}$. The fifty lanes are committed in $\qflock$, no duplicate columns needed. The twenty-six addresses are the products $\gen^{k}\fp\cdot o$.
\item \textbf{Constraints:} none. The step is handled by Flock.
\item \textbf{Flushes:}
  \begin{itemize}
  \item \textbf{State:} \pull\ $\tup{\dsep{ST},\pc,\fp}$, \push\ $\tup{\dsep{ST},\gen\cdot\pc,\fp}$.
  \item \textbf{Bytecode:} $\rdf{\dsep{BC},\pc,r_{\mathrm{bc}},\opc{S3},o_{m_0},o_{m_1},o_{m_2},o_{m_3},o_{\mathit{tail}},o_{\mathit{cap}},o_{\mathit{out}}}$.
  \item \textbf{Memory:} with $s_\ell$ the input lane $\ell$, $t_\ell$ the output lane $\ell$, and one count per cell,
    \begin{align*}
      &\rdf{\dsep{MEM},\fp\cdot o_{m_i},r,s_{2i},s_{2i+1},0}\quad(i<4),\\
      &\rdf{\dsep{MEM},\gen^{i}\cdot\fp\cdot o_{\mathit{tail}},r,s_{8+2i},s_{9+2i},0}\quad(i<4),\\
    ...
    \end{align*}
    and the same thirteen for the output lanes $t_\ell$ at $\gen^{k}\cdot\fp\cdot o_{\mathit{out}}$, in the same order.
  \end{itemize}
\end{itemize}
