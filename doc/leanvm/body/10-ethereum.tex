...
A signature claim identifies what was signed and by whom. For XMSS~\cite{rfc8391}, it is a public key, an epoch, and a message; the keys claiming one $(\text{epoch},\text{message})$ pair form one group, so an epoch signed at under several messages contributes one group per message. For SPHINCS+~\cite{sphincs}, it is a $(\text{key},\text{message})$ pair; the instance is the sphincs-g EVM profile over Keccak-256 (specified in \texttt{doc/sphincs}), which the guest absorbs with the same \texttt{SHA3} instruction, 136 bytes a block.
...
The DA claim is a sorted, duplicate-free list of at most $16$ LeanDA roots, possibly empty. Each root is paired with the hash of its membership test vector $\dual$ (\S\ref{sec:da-membership}). 

\subsubsection{The public input}\label{sec:rec-stmt}

The proof carries the signature claims and DA root list. Their digests bind these variable-length lists into a fixed-size statement. The VM's public input is the hash (SHA3-256 in the cell encoding, \S\ref{sec:tab-sha3}) of the following fields, in order:
\begin{center}
\begin{tabular}{@{}rl@{}}
\hline
...
$34\times24$ bytes & deferred matrix point and two values\\
\hline
\end{tabular}
\end{center}
...
The DA list digest is the hash of the concatenated pairs $(\daroot,\davechash)$, each a $32$-byte DA root $\daroot$ followed by a $32$-byte hash $\davechash$ of the membership test vector $\dual$.

The signer-set digest is the hash of the following, where each cell is $16$ bytes and a count is encoded as $\gen^{\text{count}}$ (8 bytes, zero-padded to one cell):
\begin{center}
\begin{tabular}{@{}ll@{}}
\hline
header & XMSS group count, SPHINCS+ claim count, SPHINCS+ list digest\\
per XMSS group & epoch, key count, $32$-byte message\\
& key list digest, two zero cells\\
\hline
\end{tabular}
\end{center}
Each inner list digest is the hash of the list alone: $32$ bytes per XMSS key or $64$ bytes per SPHINCS+ $(\text{key},\text{message})$ pair. The header is four cells and a group eight, so every hash block of the signer-set string but the last is whole, and the last is four cells whatever the group count; the lists are four keys or two claims a block, so only their last block's shape depends on their length (\S\ref{sec:prog-hashing}).
...
Verifying a child requires evaluations of three fixed polynomials: the bytecode multilinear, in $\kbc+4$ variables (\S\ref{sec:e2e-bc}), and flock's R1CS matrix multilinears $\widetilde A_0,\widetilde B_0$, each in $32$ variables (Annex~\ref{annex:flock}). Computing them inside the guest would be very expensive. Instead, the guest carries evaluation claims in its statement. These expensive computations are moved 'outside of the snark'.
...
Our construction uses SHA3-256 and the additive Reed-Solomon encoder over $\K$ already used by the PCS, with a different codeword membership test from LeanDA's (\S\ref{sec:da-membership}). The polynomial basis, transform, and membership lemmas are collected in Annex~\ref{app:ntt}.
...
Let $H$ denote the hash and $e_{i,j}$ the hash of cell $j$ in row $i$. The same cell digests feed two branches:
...
The guest recomputes $\daroot$ from the encoded rows, receives $\davec$ as a hinted vector, checks its hash against $\davechash$, and uses it to check membership:
\[
  \inner{\davec}{w_i}
...
Recursive nodes preserve the pair $(\daroot,\davechash)$ for each retained claim. The final native verifier recomputes $\davechash$ from every retained root (outside of the snark).
