...
\texttt{SHA3}           & $\opc{S3}$  & $o_{m_0}$ & $o_{m_1}$ & $o_{m_2}$ & $o_{m_3}$ & $o_{\mathit{tail}}$ & $o_{\mathit{cap}}$ & $o_{\mathit{out}}$\\
\hline
\end{tabular}
\end{center}
...
The first two memory cells must hold the public input, $\mem[\gen^{0}]=\textsf{input}_0+\textsf{input}_1y$ and $\mem[\gen^{1}]=\textsf{input}_2+\textsf{input}_3y$ (\S\ref{sec:vm}). They sit at $(\underbrace{0,\dots,0}_{\kmem})$ and $(1,\underbrace{0,\dots,0}_{\kmem-1})$, so it suffices to check the following (degree 1) polynomial equality:
\[
...
The Fiat-Shamir initial state (\S\ref{sec:e2e-fiat-shamir}) is seeded by the public input, the bytecode and Flock's two R1CS matrices, BLAKE2s's and Keccak's. The prover sends the memory log-size $\kmem$ and the seven table log-heights $\tau_j$; the verifier checks them against the caps (\S\ref{sec:lookup}) and derives the layout of every stack (PCS and GKR leaves).

\paragraph{Commitment.}

\begin{enumerate}
\item \textbf{Prover.} Stack (\S\ref{sec:stacking}) every column, the three memory limbs, the two finalize (memory / bytecode) counts, and Flock's two packed witnesses $\qflock$ (\S\ref{sec:tab-blake2s}, \S\ref{sec:tab-sha3}), into $q$ and sends its commitment (Annex~\ref{annex:pcs}).
...
\paragraph{Table sumcheck.} One run proves the seven tables' constraints together with their share of the bus (\S\ref{sec:air}).
\begin{enumerate}
...
\paragraph{SHA3 validity.}

\begin{enumerate}
\item \textbf{Prover \& Verifier.} Likewise reduce the executed Keccak constraints to one evaluation claim on the Keccak $\qflock$ (\S\ref{sec:tab-sha3}, Annex~\ref{annex:flock}), right after the BLAKE2s reduction on the same transcript.
\item \textbf{Prover \& Verifier.} Ring switching turns it into a weighted-sum claim on that region of the stack, the ring-switching map shared with the BLAKE2s claim; it joins the pool.
\end{enumerate}

\paragraph{Opening.}

\begin{enumerate}
\item \textbf{Prover \& Verifier.} Each pooled claim is one evaluation of a column at a point, its value already supplied; via the stacking selectors (\S\ref{sec:stacking}) it becomes a weighted sum $\sum_w W_j(w)\,\widetilde q(w)=c_j$ over the stacked witness. (The ring-switched claims arrive already in this form, each weight supported on its $\qflock$ region.)
\item \textbf{Verifier.} Sends one batching challenge $\lambda\in\E$, drawn after every claim value is bound. Claim $j$ takes the weight $\lambda^{j-1}$, the two ring-switched claims first (\texttt{BLAKE2S}'s, then \texttt{SHA3}'s) and the pooled column claims after: the $J$ weights are distinct powers of the one challenge (Theorem~\ref{thm:rbr}).
