...
The Fiat-Shamir initial state (\S\ref{sec:e2e-fiat-shamir}) is seeded by the public input, the bytecode and Flock's BLAKE2s R1CS matrices. The prover sends the memory log-size $\kmem$, the six table log-heights $\tau_j$, the PCS rate and the final clock $\tsfinal\in\K$ (\S\ref{sec:state}); the verifier checks the sizes against the caps (\S\ref{sec:lookup}) and derives the layout of every stack (PCS and GKR leaves).

\paragraph{Commitment.}

\begin{enumerate}
\item \textbf{Prover.} Stack (\S\ref{sec:stacking}) every column, the two memories $\meminit$ and $\memfin$, the final timestamps $\tsfin$ (\S\ref{sec:memchan}), the three finalize counts (bytecode and the two range arrays, \S\ref{sec:rangecheck}), and Flock's packed witness $\qflock$ (\S\ref{sec:tab-blake2s}), into $q$ and sends its commitment (Annex~\ref{annex:pcs}).
