...
\texttt{XOR192}        & $\opc{XOR192}$ & $o_A$ & $o_B$ & $o_C$ & $0$ & $0$ & $0$ & $0$\\
\texttt{MUL192}        & $\opc{MUL192}$ & $o_A$ & $o_B$ & $o_C$ & $0$ & $0$ & $0$ & $0$\\
\hline
\end{tabular}
\end{center}
...
The public input is the first four memory cells $\mem[\gen^{0}],\dots,\mem[\gen^{3}]$, four words both parties know. The verifier samples $r=(r_0,r_1)\in\E^{2}$ and, with nothing sent, pools the claim
\[
  \mle{\mem}(r_0,r_1,0,\dots,0)\;\qeq\;\sum_{b\in\cube{2}}\eq(r,b)\,\mem[\gen^{\,b_0+2b_1}],
\]
whose right side, the multilinear extension of the four public words at $r$ ($r_0$ on the low bit), the verifier computes itself. By Fact~\ref{fact:eq} the left side is the same sum over the four committed cells, so a mismatch leaves a nonzero multilinear polynomial in $r$, of total degree at most two, and the claim holds with probability at most $2/|\E|$ (Lemma~\ref{lem:sz}). The PCS opening binds the claim to the committed memory.
...
\begin{enumerate}
\item \textbf{Prover.} Stack (\S\ref{sec:stacking}) every column, the memory $\mem$, the two finalize (memory / bytecode) counts, and Flock's packed witness $\qflock$ (\S\ref{sec:tab-blake2s}), into $q$ and sends its commitment (Annex~\ref{annex:pcs}).
...
\item \textbf{Prover \& Verifier.} Pool that value as a claim on $\mem$ at $(r_0,r_1,0,\dots,0)$; the prover sends nothing (\S\ref{sec:e2e-pi}).
\end{enumerate}
