...
\]
A run of three consecutive cells spells one element of $\E$, its limbs low first:
\[
  \locE{o}\;=\;\loc{o}+\loc{\gen\cdot o}\,y+\loc{\gen^{2}\cdot o}\,y^{2}\;\in\;\E.
\]
...
\texttt{XOR192}        & $[o_A,o_B,o_C]$ & $\locE{o_C}=\locE{o_A}+\locE{o_B}$ \quad(addition in $\E$)\\
\texttt{MUL192}        & $[o_A,o_B,o_C]$ & $\locE{o_C}=\locE{o_A}\cdot\locE{o_B}$ (multiplication in $\E$)\\
\hline
\end{tabularx}
\end{center}
