...
\end{tabularx}
\end{center}
The three sizes are constants of the program. Everything lies in one $2$\,GiB window below $\mathtt{0x7FFF\_F800}$, which is what the \texttt{medany} and \texttt{medlow} code models and \texttt{lui} can address. Loads and stores must be naturally aligned, each touching one word; the address of a load or a store is computed modulo $2^{64}$ as the specification says, and it is that address that has to fall in RAM or the advice. The text is not data: a load from it is an access outside memory.
