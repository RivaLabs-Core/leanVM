# SHA3 over slices: `buf[lo:hi]` (2 cells) is a 256-bit operand under 128-bit
...
# public_input: 2794517349668093800472953214118377673, 331793389724556758034430547592652439197
...
def main():
    ...
    x = 0
    h = StackBuf(2)
    sha3(a[x:x + 2], hb[0:2], h)  # stack slice + heap input slice
    sha3(h, a[0:2], hb[2:4])  # digest lands in heap cells g^2, g^3
    p = GEN ** 0
    p[1] = hb[GEN ** 2]
    p[GEN] = hb[GEN ** 3]
    return
