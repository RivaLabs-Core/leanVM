...
# heap cells `buf·i·g^k`, k < 2 (one MUL folds `i` into the pointer). A SHA3
# chain over heap pairs (256-bit SHA3 digest = two canonical cells),
...
def main():
    ...
    for i in mul_range(1, GEN ** 3):
        ...
        sha3(buf[b:b + 2], buf[b:b + 2], buf[b * GEN ** 2:b * GEN ** 2 + 2])
    p = GEN ** 0
    p[1] = buf[GEN ** 6]
    p[GEN] = buf[GEN ** 7]
    return
