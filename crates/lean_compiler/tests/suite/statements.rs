...
def main():
    ...
    return
";
    compile(&parse(swap).expect("parse")).execute(pi(&[F64(7), F64(5)]));
}
...
/// that the literal no longer fits in a word, while the modulus would fold the
/// monomial back into one. A g-power recognizer without that guard reads `2^64`
/// as `g^64`, so a literal that is no machine word compiles as a value, and only
/// in the spelling that goes through the recognizer. Both spellings must be
/// rejected, and `2^63` must still be `g^63`.
#[test]
fn a_power_of_two_literal_is_a_g_power_only_below_two_to_the_64() {
    let prog = |body: &str| format!("Y_TOWER = 18446744073709551616\n\ndef main():\n    {body}\n    return\n");
    for body in [
        "yt = Y_TOWER\n    a = yt * GEN\n    assert a == a",
        "a = Y_TOWER * GEN\n    assert a == a",
    ] {
        let ast = parse(&prog(body)).expect("parse");
        let Err(err) = std::panic::catch_unwind(|| compile(&ast)) else {
            panic!("`{body}` was accepted");
        };
        let msg = crate::common::panic_message(&*err);
        assert!(msg.contains("does not fit in a 64-bit word"), "`{body}`: got `{msg}`");
    }
    let top = "a = 9223372036854775808 * GEN\n    assert a == GEN ** 64";
    compile(&parse(&prog(top)).expect("parse")).execute([F64::ZERO; 4]);
}
...
"
    ...
    let run = |src: &str| {
        ...
        program.set_witness("m", vec![vec![g_pow(5)]]);
        let want = pi(&[g_pow(5), g_pow(0)]);
        let (proof, _) = prove(&program, want, lean_vm::pcs::TEST_LOG_INV_RATE);
...
";
    ...
    program.set_witness("w", vec![vec![g_pow(1)], vec![g_pow(2)], vec![g_pow(3)]]);
    let want = pi(&[g_pow(2), g_pow(0)]);
    let (proof, _) = prove(&program, want, lean_vm::pcs::TEST_LOG_INV_RATE);
...
";
    ...
    program.set_witness("w", vec![vec![g_pow(7)], vec![g_pow(8)]]);
    let want = pi(&[g_pow(1), g_pow(0)]);
    let (proof, _) = prove(&program, want, lean_vm::pcs::TEST_LOG_INV_RATE);
    ...
    program.set_witness("m", vec![vec![g_pow(6)]]);
    let want = pi(&[g_pow(6), g_pow(0)]);
    let (proof, _) = prove(&program, want, lean_vm::pcs::TEST_LOG_INV_RATE);
...
";
    ...
    program.set_witness("w", vec![vec![F64(1234), F64(5678)]]);
    let want = pi(&[F64(1234), g_pow(0)]);
    let (proof, _) = prove(&program, want, lean_vm::pcs::TEST_LOG_INV_RATE);
...
";
    ...
    program.set_witness("x,y#z]w", vec![vec![F64(7)]]);
    let want = pi(&[F64(7), g_pow(0)]);
    let (proof, _) = prove(&program, want, lean_vm::pcs::TEST_LOG_INV_RATE);
...
";
    let mut program = compile(&parse(src).expect("parse"));
    program.set_witness("r", vec![vec![g_pow(0)]]);
    program.set_witness("vals", vec![(10u64..14).map(F64).collect()]);
    // `r` is 1, so the index is `K` itself: cell 1, holding 11. Reading the
    // integer view instead would name cell 2, holding 12.
    let want = pi(&[F64(11), g_pow(0)]);
    let (proof, _) = prove(&program, want, lean_vm::pcs::TEST_LOG_INV_RATE);
...
def main():
    ...
    return
";
    let exec = compile(&parse(src).expect("parse")).execute([F64::ZERO; 4]);
...
/// A callee returning a `StackBuf` flattens it into several ABI cells, so a
/// `match` join that read one cell per bound name would bind the run's FIRST cell
/// and leave the rest written where nothing reads them. A mixed `(scalar,
/// StackBuf)` return is the shape that slipped past the old all-scalar guard, and
/// with runs allowed across the join it is the shape to pin: every cell of the
/// run must reach the caller, through an `@inline` arm and through the fused
/// dispatch of a plain one, with a run argument going in as well.
#[test]
fn a_mixed_stack_buf_return_crosses_a_dispatched_join_whole() {
    let src = "\
@INLINE
def f(v: StackBuf(2), k: Const):
    s = StackBuf(2)
    s[0] = v[0] * GEN ** k
    s[1] = v[1] * GEN ** k
    return GEN ** k, s
...
def main():
    ...
    assert log x < 2
    pair = [GEN ** 7, GEN ** 9]
    a, b = match(log(x), range(0, 2), lambda i: f(pair, i))
    p = GEN ** 0
    p[1] = a
    p[GEN] = b[0]
    p[GEN ** 2] = b[1]
    return
";
    for decorator in ["@inline\n", ""] {
        let mut program = compile(&parse(&src.replace("@INLINE\n", decorator)).expect("parse"));
        program.set_witness("x", vec![vec![g_pow(1)]]);
        let want = pi(&[g_pow(1), g_pow(8), g_pow(10)]);
        let (proof, _) = prove(&program, want, lean_vm::pcs::TEST_LOG_INV_RATE);
        verify(&program, &want, &proof).unwrap_or_else(|e| panic!("`{decorator}`: {e:?}"));
        let mut bad = want;
        bad[2] = g_pow(9);
        assert!(
            verify(&program, &bad, &proof).is_err(),
            "`{decorator}`: the run's second cell is bound"
        );
    }
}
