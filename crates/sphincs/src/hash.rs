...
/// A hash output truncated to `n` bytes (the verifier's `nMask`).
...
/// `H_msg = keccak256(0xFF…FF ‖ R ‖ pkSeed ‖ pkRoot ‖ M)`, 112 bytes, in full. The
/// three `n`-byte values are packed, not top-aligned words.
pub fn h_msg(pp: &PublicParam, root: &Digest, r: &Randomizer, m: &Message) -> [u8; 32] {
    let mut input = [0u8; HMSG_INPUT_BYTES];
