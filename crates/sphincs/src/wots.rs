...
//!
//! A layer's WOTS key signs an `n`-byte node directly (FIPS 205 `base_2^b`): its
//! `len1 = 32` base-16 digits, then the `len2 = 3` base-16 digits of the
//! checksum `sum(w - 1 - digit)`, both most significant first. Chain `i` starts
//! at the revealed value, at hash address `digit_i`, and walks the
//! `w - 1 - digit_i` remaining steps.
