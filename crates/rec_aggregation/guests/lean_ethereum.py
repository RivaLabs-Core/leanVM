...
# SP_BIT_INDEX[b]: where bit b of the digest, read as a big-endian uint256, sits
# in the bit buffer (byte 31 - b/8, bit b mod 8, the buffer holding lanes 1..3).
SP_BIT_INDEX = SP_BIT_INDEX_PLACEHOLDER
...
def challenge_from_state(state):
    # Both words are hash outputs with zero top limbs.
...
def open_stacked(m_idx: Const, fs0, fs1, target, commit_root_0, commit_root_1, cursor):
    ...
    #
    # The returned point is in witness order. Round-order challenges stay local
    # for the induced weights, OOD claims, and residual evaluation.
...
def check_opening_terminal(zeta, chi, rm, g_bus_mu, g_zc_n, g_log_mem, tau_sha3_g, claim_cplen_g, lam_pool, col_offsets, col_kappas, z_vals, c_table, point, inner_total, yr_at_tail, sumcheck_target):
    ...
    for xk in mul_range(GEN, g_log_mem):
        pi_chain[xk * GEN] = pi_chain[xk] * (1 + point[xk])
    pi_eq = pi_chain[g_log_mem]

    selectors = StackBuf(N_COMMITTED_COLS)
    for c in unroll(0, N_COMMITTED_COLS):
        offset = col_offsets[GEN ** c]
        kappa_g = col_kappas[GEN ** c]
        selectors[c] = match(log(kappa_g), range(0, N_COLUMN_LOGS), lambda kappa: column_selector(offset, point, kappa))
...
def verify_sub(pi_0, pi_1, seed_0, seed_1, g_logs_pow2, g_squares, defer_out):
    ...
    sumcheck_target, point, inner_total, yr_at_tail = match(log(config_sel), range(0, LIG_N_CANDIDATES), lambda m_idx: open_stacked(m_idx, fs[0], fs[1], target, commit_root_0, commit_root_1, cursor))
...
@inline
def sp_bind_lane(bits, off: Const, value):
    # bind_bits for one 64-bit lane held in a StackBuf at `off`: each store is the
    # booleanity pin (a second write of the cell), one MUL.
    acc = 0
    for i in unroll(0, BASE_FIELD_BITS):
        b = bits[off + i]
        bits[off + i] = b * b
...
def sp_field(bits, off: Const, n: Const, lsb_byte: Const):
    ...
    acc = 0
    for j in unroll(0, n):
        acc += bits[SP_BIT_INDEX[off + j]] * COORD_BASIS[8 * (lsb_byte - j // 8) + j % 8]
    return acc
...
def verify_sig_sphincs(signer):
    ...
    for lane in unroll(0, SP_BIT_LANES):
        off = lane * BASE_FIELD_BITS
        hint_decompose_bits(bits[off:off + BASE_FIELD_BITS], lanes[lane], BASE_FIELD_BITS)
        sp_bind_lane(bits, off, lanes[lane])
    ...
    for i in unroll(0, SP_K):
        ...
        # GEN once the outgoing bit is cleared. The first shift also moves bit 8
        # down from byte 14 to bit 7 of byte 15.
        b0 = bits[SP_BIT_INDEX[i * SP_A]]
        b8 = bits[SP_BIT_INDEX[i * SP_A + SP_A - 1]]
        shifted = (placed + b0 * COORD_BASIS[120] + b8 * COORD_BASIS[112]) / GEN + b8 * COORD_BASIS[127]
        for z in unroll(1, SP_A + 1):
            # Height z: word2 = z, word3 = (i << (a - z)) | (index >> z). The
            # children are ordered branchlessly (see `order_children`), each
            # evaluated straight into the hash's input run.
