        ...
        loop {
            ...
            match op {
                ...
                Op::Sha3 { .. } => {
                    ...
                    // not balance the bus.
                    for (c, (w, &(_, hi))) in input.iter().zip(&CELL_LANES).enumerate() {
                        if w.c2 != 0 || (hi.is_none() && w.c1 != 0) {
                            panic!(
                                "SHA3 input cell {c} is not a canonical {}-bit embedding at pc {pc} (in {}): \
                                 {:016x}:{:016x}:{:016x}",
                                if hi.is_none() { 64 } else { 128 },
                                self.site_at(pc),
                                w.c0,
                                w.c1,
                                w.c2
