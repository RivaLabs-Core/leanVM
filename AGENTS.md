...
A minimal virtual machine and the SNARK that proves its execution. Proofs are not zero knowledge. The programs are small for now: Fibonacci, a BLAKE2s hash chain. Memory is read-write (below); the zkDSL predates that and is unsound for now.
...
Heavy benches and measurement harnesses are `#[ignore]`d; run by name with `-- --ignored --nocapture`: `hash_batch_prove_verify`, `mul_wrapping_prove_verify`, `mul_widening_prove_verify`, `pcs_throughput`, `multithreaded_throughput`, `print_whir_query_counts`, `print_whir_query_table`.
...
- **A failed zkDSL `assert` surfaces as a write-once memory conflict** in witness generation (the proof itself no longer enforces it, see Read-write memory), not an assertion message, but it names the source line: `write-once conflict at cell 34 (line 2906, pc ...)`. A failed range check and a wild `DEREF` name the function and line instead (`in verify_sub (line 2204)`). Parse and lowering errors carry a line too. Reach for `lean_compiler::disassemble` only when the line is not enough, or when the pc lands in a fill block, which has no source line by construction.
