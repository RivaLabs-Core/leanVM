...
- `formal/xmss/` and `formal/sphincs/` contain Lean 4 proofs over VCVio. XMSS proves 127-bit classical security. SPHINCS proves correctness and completeness; its security claim, counting hashes made by the adversary and final verifier, is not yet proved. See each project's `README.md` for the proof status. `Scheme.lean` contains the parameters, hash inputs and algorithms; `Statement.lean` defines the SUF-CMA game, query budget and security claim. `lake exe cache get` once, then `lake build`.
