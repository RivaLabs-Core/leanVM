...
- `doc/sphincs/` is the standalone specification of the concrete SPHINCS+ instance used where statelessness matters; its root is `doc/sphincs/main.tex`, built the same way as `doc/xmss`, and implemented by `crates/sphincs`. It is the "sphincs-g" parameter set in its compact `H_msg` revision, byte-compatible with the EVM verifier `SphincsVerifier` (format pinned to Sphincs-G `bea9447d`), a revision of NiceTry's `SphincsVerifier_v2.sol`: standard FORS under a standard WOTS+ hypertree (`d = 5`, `a = 9`, `k = 19`, `w = 16`), Keccak-256 (not the leanVM hash) over 32-byte words, FIPS 205 addresses, 6,176-byte signatures. `crates/sphincs/tests/vectors/` pins a vector the crate signed and that verifier accepts.
- `formal/xmss/` and `formal/sphincs/` are Lean 4 proofs (over VCVio) of the ideal schemes' classical random-oracle security, `xmss_has_127_bits_of_classical_security` and `sphincs_has_127_bits_of_classical_security`; `formal/sphincs/` also proves correctness and completeness, `sphincs_is_correct` and `sphincs_is_complete`, stated in `SphincsSecurity/Completeness.lean`. **`formal/sphincs/` describes the previous leanVM SPHINCS variant (WOTS+C/FORS+C over the leanVM hash), not the sphincs-g profile `crates/sphincs` now implements.** Each project's `Scheme.lean`, under `XmssSecurity/` or `SphincsSecurity/`, contains the concrete parameters, the byte layout of every hash input, and the three algorithms; `Statement.lean` imports it and defines the SUF-CMA game, hash-query budget, and security claim. `lake exe cache get` once, then `lake build`.
- The one hash function is SHA3-256 in the cell encoding, in `primitives::hash`: plain SHA3-256 up to 128 bytes, and past that SHA3-256 of the message with the fixed eight bytes `0^7 ‖ 0x80` after every 128, so that a block is eight whole 16-byte VM cells where SHA3's 136-byte rate would split one (the gap is lane 16, which the opcode supplies itself). Scalar, streaming, and a lane-transposed batched form for the PCS Merkle tree (AVX-512, AVX2, NEON with the SHA3 extension). The VM's `SHA3` opcode is one sponge step, Keccak-f\[1600\] after XORing the padding's last bit into lane 16, over a state held in thirteen memory cells; a program hashes any length by XORing each block's message into the previous state and placing the padding byte in the last block, which depends only on where the message ends, so a hash of runtime length needs only its last block dispatched.
...
| `flock`           | batched R1CS over GF(2) for Keccak-f: zerocheck + lincheck              |
| `lean_vm`         | arithmetization: tables, bus, constraints, `cpu::prove`/`verify`       |
| `lean_compiler`   | zkDSL (Python subset) → ISA                                            |
| `xmss`            | XMSS over SHA3-256; an independent leaf, consumed only by `rec_aggregation` |
...
`aggregate` takes a count per scheme, both defaulting to zero, so either alone or a mix of the two is one command; `recursion --sphincs-per-leaf` likewise puts both schemes in one tree. One SPHINCS verification uses about 1,600 Keccak-f permutations (data-dependent: 217 hash calls plus the WOTS chain steps) against XMSS's 138; use the benchmark output to compare complete VM cycle counts. `aggregate --blobs` adds LeanDA blobs, and `recursion --blobs-per-leaf` includes them in each child.
